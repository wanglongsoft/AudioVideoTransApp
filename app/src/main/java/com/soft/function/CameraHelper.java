...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    //设置预览尺寸，需在
    // Camera.Parameters.getSupportedPreviewSizes范围内设置
    ...
    public CameraHelper() {
        ...
        mCameraId = Camera.CameraInfo.CAMERA_FACING_BACK;//默认调用后置摄像头
    ...
    private void initHandlerThread() {
        ...
        handler = new Handler(handlerThread.getLooper()) {
            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                if(FINISH_DATA == msg.what) {
                    pictureRawCaptureListener.onCapture(null, -1, -1, -1);
                } else if(SEND_DATA == msg.what) {
                    pictureRawCaptureListener.onCapture((byte[]) msg.obj, WIDTH, HEIGHT, video_rotation);
                } else {
                    Log.d(TAG, "handleMessage: message abnormal");
                }
            }
        };
    }
    ...
    public void stopPreview() {
        ...
        if(!isStartPreview) {
            Log.d(TAG, "stopPreview isStartPreview is false，return");
            return;
        }
        isStartPreview = false;
        if (mCamera != null){
        ...
        Message message = handler.obtainMessage(FINISH_DATA);
    ...
    public interface OnPictureRawCapture{
        void onCapture(byte[] data, int width, int height, int video_rotation);
    }
